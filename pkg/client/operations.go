...
func (c *Client) Read(ctx context.Context, fileHandle []byte, offset int64, count int) ([]byte, bool, error) {
    if count <= 0 {
        ...
        count = 10 * 1024 * 1024 // 10MB for safety
    }
...
func (c *Client) Create(ctx context.Context, dirHandle []byte, name string, attrs *api.FileAttributes, mode api.CreateMode) ([]byte, *api.FileAttributes, error) {
    ...
    if attrs == nil {
        attrs = &api.FileAttributes{
            Mode: 0666, // Default to rw-rw-rw-(Because our users are considered to be other users by default)
        }
    }
    ...
    req := &api.CreateRequest{
        ...
        Attributes: attrs,
        Mode:       mode,
        Verifier:   uint64(time.Now().UnixNano()), // TODO: Use current time as verifier
    }
...
func (c *Client) LookupPath(ctx context.Context, path string) ([]byte, error) {
    ...
    // Normalize the path by ensuring it starts with a '/' and removing any trailing '/' (unless it is the root path).
    if !strings.HasPrefix(path, "/") {
        path = "/" + path
    }
    
    // If the path is the root directory, then directly return the root directory handle.
    if path == "/" {
    ...
    if err != nil {
        return nil, fmt.Errorf("Unable to obtain root directory handle: %w", err)
    }
    ...
    for _, component := range components {
        ...
        if err != nil {
            return nil, fmt.Errorf("Failed to find path component '%s': %w", component, err)
        }
